package net.symbiosis.common.structure;
...
public class Try<T, K> {
    ...
    public static <T, K> Try<T, K> fail(K error) {
        return new Try<>(null, error);
    }

    //this unused parameters is sometimes necessary to give the type inference a hope
    public static <T, K> Try<T, K> fail(K error, Class<T> type) {
        return new Try<>(null, error);
    }

    public static <T, K> Try<T, K> success(T val) {
        return new Try<>(val, null);
    }

    public static <T, K> Try<T, K> success(T val, Class<K> type) {
        return new Try<>(val, null);
    }
    ...
    public Try<T, K> onSuccess(Function<T, Try<T, K>> function) {
        return isSuccess() ? function.apply(value()) : this;
    }
