package net.beyondtelecom.gopay.bt_api.helper;
...
public class ValidationHelper {
    ...
    public static BTResponseObject<bt_wallet> validateMerchant(Long merchantId) {
        if (merchantId == null) {
            return new BTResponseObject<bt_wallet>(INPUT_INCOMPLETE_REQUEST)
                    .setMessage("merchantId cannot be null");
        }
        bt_wallet merchant = getEntityManagerRepo().findById(bt_wallet.class, merchantId);
        if (merchant == null) {
            return new BTResponseObject<bt_wallet>(INPUT_INVALID_WALLET)
                    .setMessage(format("Merchant with id %d was not found", merchantId));
        }

        return new BTResponseObject<>(SUCCESS, merchant);
    }
