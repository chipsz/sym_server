package net.beyondtelecom.gopay.bt_authentication.test;
...
@Test
public class BeyondTelecomAuthenticatorTest {

	@BeforeClass
	public void setUp() throws Exception {
        new ClassPathXmlApplicationContext("test-bt_authentication-spring-context.xml");
	}
...
//		String encryptedData = encryptAES(aes128BitKey, initializationVector, plainTextData);
//		System.out.println("Encrypted \"" + plainTextData + "\" to " + encryptedData);
//		assertNotNull(encryptedData);
//		String decryptedData = decryptAES(aes128BitKey, initializationVector, encryptedData);
...
//
//		//channels = WEB, POS
//		bt_user symUser1 = new bt_user(genUsername(), genUsername(), new Date(), genUsername(),
...
//
//		//channels = USSD
//		bt_user symUser2 = new bt_user(genUsername(), genUsername(), new Date(), genUsername(),
...
//
//		//joe
//		bt_auth_user testAuthUser1 = new bt_auth_user(symUser1, fromEnum(WEB),
//                fromEnum(WEB_ADMIN),null,null,new Date(),new Date(),new Date()).save();
//		bt_auth_user testAuthUser2 = new bt_auth_user(symUser1, fromEnum(POS_MACHINE),
//                fromEnum(WEB_AGENT),null,null,new Date(),new Date(),new Date()).save();
//
//		//mj
//		bt_auth_user testAuthUser3 = new bt_auth_user(symUser2,fromEnum(USSD),
...
//
//		for (int c = 0; c < BeyondTelecomSecurityEncryption.MAX_PASSWORD_TRIES; c++) {
//			assertEquals(authenticateUser(fromEnum(USSD), symUser2.getUsername(), "1234", true).getResponseCode(), AUTH_INCORRECT_PASSWORD);
//		}
...
//		bt_auth_user result = getAuthUserDao().findByUsernameAndChannel(testAuthUser2.getUser().getUsername(), fromEnum(POS_MACHINE));
...
//		BTResponseObject<bt_auth_user> userResponse = getUserByUsername(symUser1.getEmail(), fromEnum(WEB));
...
//		BTResponseObject<bt_auth_user> userResponse = getUserByUsername(symUser1.getMsisdn(), fromEnum(WEB));
//		assertTrue(userResponse.getResponseCode().equals(AUTH_NON_EXISTENT));
//
