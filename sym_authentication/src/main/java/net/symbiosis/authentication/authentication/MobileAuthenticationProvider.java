	...
	public SymResponseObject<sym_auth_user> registerMobileUser(sym_user newUser, String imei, sym_auth_group authGroup, sym_company company) {
		...
		try {
			...
			sendEmail(getSymConfigDao().getConfig(CONFIG_SYSTEM_NAME), new String[]{newUser.getEmail(),
					  ...
					  getSymConfigDao().getConfig(CONFIG_SYSTEM_NAME) + " Wallet Payment Platform", registrationEmail.toString(), CONTENT_TYPE_HTML);
		} catch (Exception e) {
			e.printStackTrace();
		...
		if (passResponse.getResponseCode().equals(SUCCESS)) {
			...
			try {
				...
				sendEmail(getSymConfigDao().getConfig(CONFIG_SYSTEM_NAME), new String[]{authUser.getUser().getEmail(),
						  ...
						  getSymConfigDao().getConfig(CONFIG_SYSTEM_NAME) + " Wallet Platform", registrationEmail.toString(), CONTENT_TYPE_HTML);
			} catch (Exception e) {
				e.printStackTrace();
