...
public class EnumEntityRepoManager {
    ...
    private static HashMap<String, HashMap<String, bt_enum_entity>> ENUM_ENTITY_CACHE = new HashMap<>();

    public static <T extends bt_enum_entity> List<T> findEnabled(Class<T> entityClass) {
        ...
        logger.warning(format("Found %s %s's in status enabled", results.size(), entityClass.getSimpleName()));

        if (results.size() > 0) {
            if (ENUM_ENTITY_CACHE.get(entityClass.getSimpleName()) == null) {
                HashMap<String, bt_enum_entity> cachedValues = new HashMap<>();
                for (bt_enum_entity gopayEnumEntity : results) {
                    cachedValues.put(gopayEnumEntity.getName(), gopayEnumEntity);
                }
    ...
    public static <T extends bt_enum_entity> T findByName(Class<T> entityClass, String enumName) {
