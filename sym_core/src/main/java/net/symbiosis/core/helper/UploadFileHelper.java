...
public class UploadFileHelper {
	...
	public static SymResponseObject<File> decryptPGPFile(String filePath, sym_pin_import_config pinImportConfig) {
        ...
        } else if (pinImportConfig.getPgp_public_key_file() == null || pinImportConfig.getPgp_key_pass() == null) {
            return new SymResponseObject<>(CONFIGURATION_INVALID);
        }

	    logger.info(format("Decrypting PGP file %s with key %s", filePath, pinImportConfig.getPgp_public_key_file()));
        ClassLoader classLoader = UploadFileHelper.class.getClassLoader();
        InputStream pgpKeyFileStream = classLoader.getResourceAsStream("pgp/" + pinImportConfig.getPgp_public_key_file());
